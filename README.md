...
- [ ] 两层 Pattern Lookup（PatternEvaluator）
  - L1：窗口 -> PatternType
...
- [ ] Aspiration Windows
- [ ] IID（内部迭代加深）
- [ ] 候选分层硬规则（VCF/VCT > 必防 > 强制威胁 > killer > history）
- [ ] Threat Quiescence（只扩展五/活四/冲四/可选活三）
- [ ] Eval Correction（轻量残差，`|delta| < 主评估 10%`）

### P3：并行 + 高级战术（Week 5–8）

- [ ] Lazy SMP（shared TT，多线程独立搜索）
- [ ] VCT 扩展版（活三、双威胁）
- [ ] 禁手增量缓存（ForbiddenChecker）
